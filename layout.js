...
var json = toJSON(graph,
  node => {
    return {
        ...
        color: "rgb("
            + Math.max(200 - indegree(node.id), 0)
            + ","
            + (node.id.indexOf('story|') === 0 ? 0 : Math.max(200 - indegree(node.id), 0))
            + ","
            + Math.max(200 - indegree(node.id), 0)
            + ")",
  ...
  link => {
    return {
        ...
        attributes: {},
        color: "rgba("
            + Math.max(200 - indegree(link.fromId), 0)
            + ","
            + (link.fromId.indexOf('story|') === 0 ? 0 : Math.max(200 - indegree(link.fromId), 0))
            + ","
            + Math.max(200 - indegree(link.fromId), 0)
            + ","
            + 0.1
